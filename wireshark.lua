...
F.ack_required     = ProtoField.bytes("lifx.ack_required"    , "ack_required"         , base.HEX)
F.res_required     = ProtoField.bytes("lifx.res_required"    , "res_required"         , base.HEX)
F.sequence         = ProtoField.bytes("lifx.sequence"        , "Sequence"             , base.HEX)
F.source     	   = ProtoField.bytes("lifx.source"          , "Source"               , base.HEX)
...
function analyse(buffer, pinfo, tree)
	...
	subtree:add(F.source, buffer(4,4))
	subtree:add(F.targetAddr, buffer(8,6))
	subtree:add(F.reserved, buffer(14,2))
	subtree:add(F.site, buffer(16,6))
	subtree:add(F.ack_required, buffer(22,1))
	subtree:add(F.res_required, buffer(22,1))
	subtree:add(F.sequence, buffer(23,1))
	subtree:add_le(F.timestamp, buffer(24,8))
